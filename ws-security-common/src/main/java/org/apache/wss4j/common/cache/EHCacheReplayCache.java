...
/**
 * An in-memory EHCache implementation of the ReplayCache interface, that
 * overflows to disk.
 * The default TTL is 60 minutes and the max TTL is 12 hours.
 */
public class EHCacheReplayCache implements ReplayCache {

    private static final org.slf4j.Logger LOG = org.slf4j.LoggerFactory.getLogger(EHCacheReplayCache.class);
    ...
    public EHCacheReplayCache(String cacheKey, Path diskstorePath, Map oldconfig) throws WSSecurityException {
        ...
        if (diskstorePath != null && (diskElements < 5000 || diskElements > 10000000)) {
            throw new IllegalArgumentException(
                    "The getMaxElementsOnDisk parameter must be between 5 and 10000 (megabytes)");
        }
        if (heapEntries < 100) {
            throw new IllegalArgumentException(
                    "The getMaxEntriesLocalHeap parameter must be greater than 100 (entries)");
        }
        ...
        try {
            ...
            if (diskstorePath != null) {
                cacheManager = CacheManagerBuilder.newCacheManagerBuilder()
                        .with(CacheManagerBuilder.persistence(diskstorePath.toFile())).withCache(key,
    ...
    /**
     * Add the given identifier to the cache. It will be cached for a default amount
     * of time.
     * 
     ...
     * 
     * @param identifier The identifier to be added
     * @param expiry     A custom expiry time for the identifier. Can be null in
     *                   which case, the default expiry is used.
     */
    ...
    public synchronized void close() {
        if (cacheManager.getStatus() == Status.AVAILABLE) {
            ...
            if (!persistent && cacheManager instanceof PersistentCacheManager) {
                ...
                // As we're not using a persistent disk store, just delete it - it should be
                // empty after calling
                // destroy above
                if (diskstorePath != null) {
