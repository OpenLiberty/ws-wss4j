        ...
        try {
            ...
            if (eternal) {
                configurationBuilder = 
                    CacheConfigurationBuilder.newCacheConfigurationBuilder(
                            String.class, EHCacheValue.class, resourcePoolsBuilder).withExpiry(expiry3);
            } else {
                configurationBuilder = 
                    CacheConfigurationBuilder.newCacheConfigurationBuilder(
                        String.class, EHCacheValue.class, resourcePoolsBuilder).withExpiry(expiry);
            }
            if (diskstorePath != null) {
                cacheManager = 
                    CacheManagerBuilder.newCacheManagerBuilder().with(
                        CacheManagerBuilder.persistence(diskstorePath.toFile())).withCache(key,configurationBuilder).build();
            } else {
